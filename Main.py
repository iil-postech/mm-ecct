...
if __name__ == '__main__':
    ...
    parser.add_argument('--ecct_type', type=str, default='DM',
                        choices=['SM', 'DM'])
    ...
    parser.add_argument('--code_type', type=str, default='POLAR',
                        choices=['BCH', 'POLAR', 'LDPC', 'LTE_TURBO'])
    ...
    if args.ecct_type == 'SM':
        from Model_SM import ECC_Transformer
    elif args.ecct_type == 'DM':
        from Model_DM import ECC_Transformer
    else:
        raise Exception(f'Wrong ECCT type {args.ecct_type}')
    ...
    G, H, H2 = Get_Generator_and_Parity(code,standard_form=args.standardize)
...
class ECC_Dataset(data.Dataset):
    def __init__(self, code, sigma, len, zero_cw=True):
        ...
        self.pc_matrix2 = code.pc_matrix2.transpose(0, 1)
    ...
    def __getitem__(self, index):
        ...
        syndrome2 = torch.matmul(sign_to_bin(torch.sign(y)).long(),self.pc_matrix2) % 2
...
def train(model, device, train_loader, optimizer, epoch, LR):
    ...
    for batch_idx, (m, x, z, y, magnitude, syndrome, syndrome2) in enumerate(
            train_loader):
        ...
        if args.ecct_type == 'SM':
            ...
            z_pred = model(magnitude.to(device), syndrome.to(device),syndrome2.to(device))
        else:
            raise Exception(f'Wrong ECCT type {args.ecct_type}')
...
def test(model, device, test_loader_list, EbNo_range_test, min_FER=100):
    ...
    with torch.no_grad():
        for ii, test_loader in enumerate(test_loader_list):
            ...
            while True:
                (m, x, z, y, magnitude, syndrome,syndrome2) = next(iter(test_loader))
                z_mul = (y * bin_to_sign(x))
